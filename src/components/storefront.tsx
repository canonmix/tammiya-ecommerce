...
export default function Storefront({ products, categories }: { products: CatalogProduct[]; categories: string[] }) {
  ...
  const paymentSteps = [
    { number: "01", title: "หยิบสินค้าใส่ตะกร้า", detail: "เลือกโมเดล อะไหล่ หรือของแต่งที่ต้องการ แล้วกดใส่ตะกร้า", icon: "🛒" },
    { number: "02", title: "กด LOGIN เข้าสู่ระบบ", detail: "เข้าสู่ระบบเพื่อยืนยันข้อมูลผู้สั่งซื้อและที่อยู่จัดส่ง", icon: "↪" },
    { number: "03", title: "ตรวจสอบข้อมูล และกดสั่งซื้อ", detail: "ตรวจสอบรายการสินค้า ที่อยู่ และยอดรวมให้เรียบร้อย", icon: "✓" },
    { number: "04", title: "สแกน QR เพื่อชำระเงิน", detail: "สแกน QR Payment และชำระเงินตามยอดที่แสดงในคำสั่งซื้อ", icon: "▦" },
    { number: "05", title: "รอรับเลข Tracking ในวันถัดไป", detail: "ทีมงานจัดส่งสินค้าและแจ้งเลข Tracking ให้ในวันถัดไป", icon: "➜" },
  ];
  ...
  return <main>
    ...
    <header className="border-b border-[#e7e1d8] bg-white/95 sticky top-0 z-20 backdrop-blur"><div className="container flex h-20 items-center justify-between gap-5">
      ...
      <nav className="hidden gap-7 text-sm font-bold md:flex"><a href="#shop">สินค้า</a><a href="#payment">วิธีการชำระเงิน</a><a href="#story">เรื่องราวของเรา</a><a href="#service">บริการ</a></nav>
    ...
    <section id="payment" className="border-y border-[#e7e1d8] bg-[#f8f7f4] py-20"><div className="container"><div className="mx-auto max-w-2xl text-center"><p className="eyebrow mb-3">Easy payment journey</p><h2 className="text-4xl font-black tracking-tight">วิธีการชำระเงิน</h2><p className="mt-4 text-[#687582]">สั่งซื้อได้ง่าย ๆ ตาม 5 ขั้นตอน พร้อมรับสินค้าในวันถัดไป</p></div><div className="relative mt-12 grid gap-4 md:grid-cols-5">{paymentSteps.map((step, index) => <div key={step.number} className="relative rounded-3xl border border-[#e7e1d8] bg-white p-5 shadow-sm"><div className="mb-5 flex items-center justify-between"><span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#18212b] text-xl text-white">{step.icon}</span><span className="text-4xl font-black text-[#ef6c3d]/20">{step.number}</span></div><h3 className="min-h-14 text-base font-black leading-6">{step.title}</h3><p className="mt-3 text-sm leading-6 text-[#687582]">{step.detail}</p>{index < paymentSteps.length - 1 && <span className="absolute -right-5 top-10 z-10 hidden text-2xl font-black text-[#ef6c3d] md:block">→</span>}</div>)}</div></div></section>
