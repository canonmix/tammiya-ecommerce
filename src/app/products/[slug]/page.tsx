...
export default async function ProductPage({ params }: Props) {
  ...
  return <main><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} /><header className="border-b border-[#e7e1d8] bg-white"><div className="container flex h-20 items-center justify-between"><Link href="/" className="text-xl font-black"><Image src="/mini4wd-logo.png" alt="MINI4WD Premium Shop" width={220} height={66} className="h-10 w-auto" priority/></Link><Link href="/" className="text-sm font-bold text-[#687582]">← กลับไปเลือกสินค้า</Link></div></header><div className="container grid gap-10 py-14 md:grid-cols-2 md:items-center"><ProductGallery images={product.images} name={product.name} color={product.color}/><article><p className="eyebrow">Tamiya Product Code</p><p className="mt-3 text-2xl font-black text-[#ef6c3d]">Tamiya {product.sku}</p><h1 className="mt-3 text-4xl font-black tracking-tight">{product.name}</h1><p className="mt-5 text-lg leading-8 text-[#687582]">{product.description}</p><div className="mt-7 flex flex-wrap items-center gap-5"><span className="text-3xl font-black">{formatBaht(product.price)}</span>{product.stock > 0 ? <span className="rounded-full bg-emerald-50 px-3 py-1 text-sm font-bold text-emerald-700">มีสินค้า {product.stock} ชิ้น</span> : <span className="rounded-full bg-red-50 px-3 py-1 text-sm font-bold text-red-700">สินค้าหมด</span>}</div>{product.stock > 0 ? <Link href="/checkout" className="mt-8 inline-block rounded-full bg-[#18212b] px-7 py-3 font-bold text-white">สั่งซื้อสินค้า</Link> : <span className="mt-8 inline-block rounded-full bg-[#c7ccd1] px-7 py-3 font-bold text-white">สินค้าหมดชั่วคราว</span>}<div className="mt-10 border-t border-[#e7e1d8] pt-6 text-sm text-[#687582]"><p><b>รหัสสินค้า:</b> {product.sku}</p><p className="mt-2"><b>หมวดหมู่:</b> {product.category}</p></div></article></div></main>;
}
