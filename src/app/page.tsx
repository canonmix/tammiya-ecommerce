...
export default function Home() {
  ...
  return <main>
    ...
    <section id="shop" className="container pb-24"><div className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="eyebrow mb-3">Curated collection</p><h2 className="text-4xl font-black tracking-tight">ของที่พร้อมลงสนาม</h2></div><div className="flex flex-wrap gap-2">{categories.map((item) => <button key={item} onClick={() => setCategory(item)} className={`rounded-full px-4 py-2 text-sm font-bold ${category === item ? "bg-[#18212b] text-white" : "border border-[#d8d0c5]"}`}>{item}</button>)}</div></div><div className="mb-8"><label htmlFor="product-search" className="sr-only">ค้นหาสินค้า รหัส Tamiya หรือชื่อสินค้า</label><input id="product-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="ค้นหา เช่น Tamiya 15437, 15437 หรือ 13mm Aluminum..." className="w-full rounded-2xl border border-[#d8d0c5] bg-white px-5 py-4 text-sm outline-none focus:border-[#ef6c3d]"/></div><div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{visible.map((product) => <article key={product.id} className="group overflow-hidden rounded-3xl border border-[#e7e1d8] bg-white"><Link href={`/products/${product.slug}`} className="relative flex h-60 items-center justify-center" style={{background: product.color}}><span className="text-6xl font-black italic text-[#18212b]/80 transition-transform group-hover:scale-110">{product.name.split(" ")[0]}</span>{product.badge && <span className="absolute left-4 top-4 rounded-full bg-white px-3 py-1 text-xs font-black">{product.badge}</span>}</Link><div className="p-5"><p className="mb-2 text-xs font-bold text-[#ef6c3d]">Tamiya {product.sku} · {product.category}</p><Link href={`/products/${product.slug}`}><h3 className="text-lg font-black hover:text-[#ef6c3d]">{product.name}</h3></Link><p className="mt-2 min-h-12 text-sm leading-6 text-[#687582]">{product.description}</p><div className="mt-5 flex items-center justify-between"><span className="text-xl font-black">{formatBaht(product.price)}</span><button onClick={() => add(product.id)} className="rounded-full bg-[#18212b] px-4 py-2 text-sm font-bold text-white hover:bg-[#ef6c3d]">ใส่ตะกร้า</button></div></div></article>)}</div></section>
